...
module Crea
  class ApiTest < Crea::Test
    ...
    def test_inspect
      assert_equal "#<CondenserApi [@chain=crea, @methods=<84 elements>]>", @api.inspect
    end
    ...
    def test_all_methods
      vcr_cassette("#{@api.class.api_name}_all_methods") do
        @methods.each do |key|
          ...
          then
            assert_raises Crea::ArgumentError, "expect 1 argument to raise ArgumentError for: #{key}" do
              assert @api.send key, [nil]
            end
          when *METHOD_NAMES_2_ARGS
          then
            assert_raises Crea::ArgumentError, "expect 2 arguments to raise ArgumentError for: #{key}" do
              assert @api.send key, [nil, nil]
            end
          when *METHOD_NAMES_3_ARGS
          then
            assert_raises Crea::ArgumentError, "expect 3 arguments to raise ArgumentError for: #{key}" do
              assert @api.send key, [nil, nil, nil]
            end
          when *METHOD_NAMES_4_ARGS
          then
            assert_raises Crea::ArgumentError, "expect 4 arguments to raise ArgumentError for: #{key}" do
    ...
    def test_get_content_wrong_arguments
      vcr_cassette('condenser_api_get_content_wrong_arguments') do
        assert_raises Crea::ArgumentError, 'expect argument error' do
          @api.get_content
        end
