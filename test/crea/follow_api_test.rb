...
module Crea
  class FollowApiTest < Crea::Test
    def setup
      @api = Crea::FollowApi.new(url: TEST_NODE)
      @jsonrpc = Jsonrpc.new(url: TEST_NODE)
      @methods = @jsonrpc.get_api_methods[@api.class.api_name]
    end
    def test_api_class_name
      assert_equal 'FollowApi', Crea::FollowApi::api_class_name
    end
    
    def test_inspect
      assert_equal "#<FollowApi [@chain=crea, @methods=<10 elements>]>", @api.inspect
    end
    ...
    def test_get_follow_count
      vcr_cassette('follow_api_get_follow_count', record: :once) do
        ...
        @api.get_follow_count(options) do |result|
          ...
          assert_equal 'creary', result.account
