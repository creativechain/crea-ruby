...
module Crea
  class TagsApiTest < Crea::Test
    def setup
      @api = Crea::TagsApi.new(url: TEST_NODE)
      @jsonrpc = Jsonrpc.new(url: TEST_NODE)
      @methods = @jsonrpc.get_api_methods[@api.class.api_name]
    end
    def test_api_class_name
      assert_equal 'TagsApi', Crea::TagsApi::api_class_name
    end
    
    def test_inspect
      assert_equal "#<TagsApi [@chain=crea, @methods=<20 elements>]>", @api.inspect
    end
    ...
    def test_get_tags_used_by_author
      vcr_cassette('tags_api_get_tags_used_by_author', record: :once) do
        @api.get_tags_used_by_author(author: 'creary') do |result|
